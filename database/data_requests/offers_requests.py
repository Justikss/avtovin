...
class OffersRequester:
    ...
    async def get_by_seller_id(seller_id_value, viewed_value):
        ...
        active_offers = list(await manager.execute(ActiveOffers.select().where(
            (ActiveOffers.viewed == viewed_value) & (ActiveOffers.seller_id == seller_id_value)
        )))
        # seller = await manager.get(Seller, Seller.id == seller_id_value)
        # active_offers = await manager.prefetch(seller.active_offers)
        # result = [offer for offer in active_offers if offer.viewed == viewed_value]
...
class CachedOrderRequests:
    ...
    async def get_cache(buyer_id, brand=None, car_id=None):
        ...
        query = CacheBuyerOffers.select().where(
            (CacheBuyerOffers.buyer_id == buyer_id) & (CacheBuyerOffers.car_id == car_id) if car_id else None)
        select_query = await manager.execute(query)
        if select_query:
            if brand:

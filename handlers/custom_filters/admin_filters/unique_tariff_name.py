...
class UniqueTariffNameFilter(BaseFilter):
    async def __call__(self, message: Message, state: FSMContext):
        ...
        if not match_name_tariff:
            tariff_len_validator = ic(len(name.replace('<', '&lt;').replace('>', '&gt;'))) < ic(config_module.max_contact_info_len)
            ic(tariff_len_validator)
            if not tariff_len_validator:
                incorrect_flag = '(len)'
            ic(match_name_tariff)
        else:
            incorrect_flag = '(match)'
        ...
        if not incorrect_flag:
            ...
            return await process_write_tariff_time_duration(message, state, incorrect=incorrect_flag)

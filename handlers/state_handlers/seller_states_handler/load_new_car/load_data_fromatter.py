...
async def data_formatter(request: Union[Message, CallbackQuery], state: FSMContext, id_values=False):
    ...
    from handlers.callback_handlers.sell_part.commodity_requests.confirm_load_config_from_seller import \
        get_complectation_id
    ...
    'complectation': memory_storage.get('complectation_for_load'),
    ...
    car_complectation_id = await get_complectation_id(memory_storage, request, state)
    ic()
    ic(car_complectation_id)
    if not car_complectation_id:
        return 'exit'
    else:
        sub_data['complectation'] = car_complectation_id
    result_data = dict()
    ic(sub_data)
    if not id_values:
        ...
        for key, value in sub_data.items():
            ...
            if key not in ('seller_id', 'sum_price', 'dollar_price', 'photos') and value != None:
                if str(value).startswith('['):
                    value = eval(value)[0]
                if str(value).isdigit() or key == 'complectation':
                    ...
                    if str(value) in ('0', 'None', 'False') and key == 'color':
                        ...
                        raise Exception(f'None moment {value}')
                        # continue

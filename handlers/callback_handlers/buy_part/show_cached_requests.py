...
from handlers.state_handlers.choose_car_for_buy.hybrid_handlers import get_cars_data_pack
...
async def get_cached_requests__chose_brand(callback: CallbackQuery, state: FSMContext):
    ...
    cached_brands = await CachedOrderRequests.get_cached_brands(buyer_id=callback.from_user.id)
...
async def output_cached_requests(callback: CallbackQuery, state: FSMContext):
    ...
    cars = await CachedOrderRequests.get_cache(buyer_id=str(callback.from_user.id), brand=car_brand)
    if cars:
        await state.set_state(CheckNonConfirmRequestsStates.brand_flipping_process)
        formatted_cars_data = await get_cars_data_pack(callback=callback, state=state, car_models=cars)

...
async def get_seller_name(seller_model: Seller) -> Union[Tuple[str, str], str]:
    ...
    if seller_model.dealship_name:
        name = f'{lexicon_module.LexiconSellerProfile.dealership_prefix}\n{lexicon_module.LexiconSellerProfile.dealership_name_prefix}{seller_model.dealship_name}'
        address = f'{lexicon_module.LexiconSellerProfile.dealership_address_prefix}{seller_model.dealship_address}'
        return (name, address)
    else:
        if seller_model.patronymic:
            ...
            patronymic = ''
        name = f'{lexicon_module.LexiconSellerProfile.seller_prefix}{lexicon_module.LexiconSellerProfile.seller_name_prefix}{seller_model.name} {seller_model.surname} {patronymic}'
...
async def seller_profile_card_constructor(callback: CallbackQuery) -> str:
    ...
    output_string = f'{lexicon_module.LexiconSellerProfile.header}{seller_data}\n{lexicon_module.LexiconSellerProfile.phonenumber_prefix}{seller_model.phone_number}'
    ...
    if seller_tariff_model:
        ...
        if seller_tariff_model.end_date_time < datetime.now():
            ...
            output_string += f'\n{lexicon_module.LexiconSellerProfile.tariff_prefix} {seller_tariff_model.tariff.name}\
                {lexicon_module.LexiconSellerProfile.tariff_out_date_prefix} {seller_tariff_model.end_date_time}\
                    \n{lexicon_module.LexiconSellerProfile.residual_feedback_prefix} {seller_tariff_model.residual_feedback}'
...
async def output_seller_profile(callback: CallbackQuery):
    ...
    lexicon_part = {'message_text': await seller_profile_card_constructor(callback=callback),
                    'buttons': lexicon_module.LexiconSellerProfile.tariff_extension_button}
